...
| `route_requests.route_type` | `varchar(20)` | 경로 유형 |
...
- route_type: 경로 유형 (`varchar(20)`)
  - 세부 허용 값은 서비스 로직 / API 계약에서 관리
...
- users_idx: FK -> service.users.idx
- route_recommendations_idx: FK -> service.route_recommendations.idx
- status: enum[IN_PROGRESS / COMPLETED / STOPPED / FAILED / CANCELLED]
  - IN_PROGRESS: 현재 러닝 진행 중
  - COMPLETED: 정상 종료
  - STOPPED: 사용자가 중간 종료
  - FAILED: 앱 / GPS 오류 등 비정상 종료
  - CANCELLED: migration에서 추가된 취소 상태
  - 기본값 IN_PROGRESS

`CANCELLED`의 세부 발생 조건은 서비스 로직 / API 정책에서 정의한다.
