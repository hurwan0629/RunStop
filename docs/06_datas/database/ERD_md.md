...
- user_profiles: 사용자 프로필 및 러닝 설정 정보
  - idx: 식별자
  - users_idx: FK -> users.idx
    - 사용자별 하나의 프로필만 존재
    - UNIQUE
  - weight: 사용자 몸무게 (kg)
  - height: 사용자 키 (cm)
...
- running_goals: 사용자의 월간 / 주간 러닝 목표
  - idx: 식별자
  - users_idx: FK -> users.idx
...
- point_bookmarks: 사용자 출발지 / 목적지 / 경유지 즐겨찾기
  - idx: 식별자
  - users_idx: FK -> users.idx
  - name: 사용자가 저장한 지점 이름
  - point: 저장된 지점
...
- route_requests: 사용자가 입력한 경로 추천 요청
  - idx: 식별자
  - users_idx: FK -> users.idx
  ...
  - selected_recommendations_idx: 사용자가 최종 선택한 후보 경로
    - FK -> route_recommendations.idx
    - 후보를 선택하기 전에는 NULL
    - 해당 recommendation이 반드시 현재 route_request에서 생성된 후보인지 서비스 로직에서 검증
  - created_at: 생성일자

- route_request_points: 사용자가 경로 요청 시 지정한 출발지 / 경유지 / 목적지
  - idx: 식별자
...
- route_recommendations: 알고리즘이 생성한 후보 경로
  - idx: 식별자
  - route_requests_idx: FK -> route_requests.idx
...
- route_points: 후보 경로에 포함된 주요 지점
  - idx: 식별자
  - route_recommendations_idx: FK -> route_recommendations.idx
...
- route_bookmarks: 사용자가 즐겨찾기한 추천 코스
  - idx: 즐겨찾기 식별자
  - users_idx: FK -> users.idx
  - route_recommendations_idx: FK -> route_recommendations.idx
...
- running_sessions: 한 번의 러닝 전체 기록
  - idx: 식별자
  - users_idx: FK -> users.idx
  - route_recommendations_idx: FK -> route_recommendations.idx
...
- running_trackpoints: 러닝 중 주기적으로 수집한 GPS 위치 기록
  - idx: 식별자
  - running_sessions_idx: FK -> running_sessions.idx
  - point: 기록된 GPS 위치
    - PostGIS geometry(Point, 4326)
  - recorded_at: GPS 위치가 기록된 시각
  - accuracy: 단말 위치 API가 제공하는 위치 정확도 추정값

- inquiries: 사용자 문의사항 및 관리자 답변
  - idx: 식별자
  - users_idx: FK -> users.idx
  - title: 문의 제목
  - content: 문의 내용
