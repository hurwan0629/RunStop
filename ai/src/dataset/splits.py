"""User-level cold split plus per-user temporal warm split."""
import numpy as np
import pandas as pd
from ai.src.config.schema import SplitConfig
...
def split_user_temporal_holdout(
    ...
    parts = {
        key: df[df.request_id.isin(requests.loc[requests.split == key, "request_id"])].copy()
...
def assert_no_leakage(parts: dict[str, pd.DataFrame]) -> None:
    """Check request exclusivity, cold-user isolation, and temporal order."""
    seen = set()
