...
def recommend(G, idx, route_type, start, target_km, end=None, vias=None,
    ...
    for c in cands:
        ...
        # { avg_slope_pct, max_slope_pct, elevation_gain_m, sample_count }
        c["slope"] = slope_profile(c["coords"])          # 경사 (DEM)

        # 반환값: { route_length_km, buffer_m, {key}_count, {key}_per_km, {key}_nearest_m }
        c["facilities"] = score_route(c["coords"])        # 시설 (CSV)

        # 영역 데이터 인접률 구하기
        c["nature"] = adjacency_ratios(c["coords"])       # 녹지·하천 인접률 (OSM 폴리곤)
        # length_m, walkable_ratio, bigroad_ratio, signal_per_km, crossing_per_km
